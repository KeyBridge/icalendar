 ...
 * specify repeating events based on an interval of a second or more; MINUTELY,
 * to specify repeating events based on an interval of a minute or more; HOURLY,
 * to specify repeating events based on an interval of an hour or more; DAILY,
 * to specify repeating events based on an interval of a day or more; WEEKLY, to
 * specify repeating events based on an interval of a week or more; MONTHLY, to
 * specify repeating events based on an interval of a month or more; and YEARLY,
 ...
 * hour for an HOURLY rule, every day for a DAILY rule, every week for a WEEKLY
 * rule, every month for a MONTHLY rule, and every year for a YEARLY rule. For
 ...
 * rule part MUST also be specified as a date with local time. If the "DTSTART"
 * property is specified as a date with UTC time or a date with local time and
 * time zone reference, then the UNTIL rule part MUST be specified as a date
 * with UTC time. In the case of the "STANDARD" and "DAYLIGHT" sub-components
 * the UNTIL rule part MUST always be specified as a date with UTC time. If
 ...
 * tenth to the last day of the month. The BYMONTHDAY rule part MUST NOT be
 * specified when the FREQ rule part is set to WEEKLY.
 * <p>
 * The BYYEARDAY rule part specifies a COMMA-separated list of days of the year.
 * Valid values are 1 to 366 or -366 to -1. For example, -1 represents the last
 * day of the year (December 31st) and -306 represents the 306th to the last day
 * of the year (March 1st). The BYYEARDAY rule part MUST NOT be specified when
 * the FREQ rule part is set to DAILY, WEEKLY, or MONTHLY.
 * <p>
 ...
 * calendar year. This rule part MUST NOT be used when the FREQ rule part is set
 * to anything other than YEARLY. For example, 3 represents the third week of
 * the year.
 * <p>
 ...
 * values are MO, TU, WE, TH, FR, SA, and SU. This is significant when a WEEKLY
 * "RRULE" has an interval greater than 1, and a BYDAY rule part is specified.
 * This is also significant when in a YEARLY "RRULE" when a BYWEEKNO rule part
 ...
 * to 366 or -366 to -1. It MUST only be used in conjunction with another BYxxx
 * rule part. For example "the last work day of the month" could be represented
 * as:
 * <p>
